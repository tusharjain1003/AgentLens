# AgentLens — Resume One-Pager

> **Project:** AgentLens — Production-Grade Web Search RAG System  
...
- Designed and built a **15-node LangGraph orchestration pipeline** (query rewrite → route classification → semantic cache → URL discovery → extraction → chunking → fused retrieval/generation → reflection coverage check → claim verification → synthesize → cache insert), replacing a monolithic 500-line async coroutine with independently traceable, error-isolated nodes; each search-path node carries an error short-circuit edge to `emit_done`, so any single stage failure emits a structured error SSE event rather than crashing the coroutine.
...
|---|---|---|---|
| Orchestration | LangGraph 15-node graph | Monolithic async coroutine | LangSmith spans + conditional routing; slight startup overhead |
...
> "AgentLens retrieves and answers questions from the live web. The core insight is that search snippets (120 chars) lose critical context — instead, I extract full-page markdown, chunk it along heading boundaries, and run hybrid BM25 + dense retrieval fused with RRF, then cross-encoder reranked. The pipeline is a 15-node LangGraph graph with reflection and claim verification; each node emits SSE events so the user sees the pipeline trace live. I built an eval harness that scores 5 RAG metrics against 30 adversarial questions and used it to iterate from a 0.718 to 0.789 aggregate score across 3 versioned releases — with latency dropping 13% at the same time."
